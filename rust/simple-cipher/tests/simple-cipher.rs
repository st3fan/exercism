...
// Tests for my own functions

#[test]
fn test_encode_char() {
    assert_eq!('l', encode_char('d', 'i'));
    assert_eq!('z', encode_char('a', 'z'));
    assert_eq!('a', encode_char('b', 'z'));
}

#[test]
fn test_decode_char() {
    assert_eq!('i', decode_char('d', 'l'));
    assert_eq!('x', decode_char('d', 'a'));
    assert_eq!('y', decode_char('d', 'b'));
}
...
fn cipher_is_reversible_given_key() {
    const P: &str = "thisismysecret";
    const K: &str = "abcdefghijabcdefghij";

    let encoded = encode(K, P);
    let decoded = decode(K, &encoded.unwrap());

    assert_eq!(decoded, Some(P.to_string()));
