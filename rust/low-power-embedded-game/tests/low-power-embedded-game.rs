...
mod evens_through_specialized_iterator {
    use low_power_embedded_game::evens_through_specialized_iterator;

    #[test]
    fn simple_i32() {
        let out: Vec<i32> = evens_through_specialized_iterator(0..).take(5).collect();
        assert_eq!(out, &[0, 2, 4, 6, 8]);
    }

    #[test]
    fn reverse_i32() {
        let out: Vec<i32> = evens_through_specialized_iterator((0..=10).rev()).collect();
        assert_eq!(out, &[10, 8, 6, 4, 2, 0]);
    }

    #[test]
    fn offset_i32() {
        let out: Vec<i32> = evens_through_specialized_iterator(1..).take(5).collect();
        assert_eq!(out, &[1, 3, 5, 7, 9]);
    }
    ...
    fn strs() {
        let input = "You really must never be above joking.".split_whitespace();
        let expected: Vec<_> = "You must be joking.".split_whitespace().collect();
        let out: Vec<_> = evens_through_specialized_iterator(input).collect();
        assert_eq!(out, expected);
    }
}
